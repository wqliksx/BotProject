...
async def help(update, context):
    ...
    id = update.effective_chat.id  # 978543035
...
async def alt_first_stage(update, context):
    if third_stage_keyboard.index([update.message.text]):
        await update.message.reply_text('Чтоже, тогда какого рода деятельность вам больше всего симпатизируе'
                                        'т?', reply_markup=markup_s)
        return 2.5
    else:
        await update.message.reply_text() # сделать отказ
...
async def real_number(update, context):
    ...
    if test(num):
        ...
        await update.message.reply_text('Номер указан верно.\nКакого рода деятельность вам больше всего '
                                        'симпатизирует?', reply_markup=markup_s)
        return 2
    else:
...
async def alt_fin_stage(update, context):
    ...
    db_sess.commit()
    if third_stage_keyboard.index([update.message.text]):
        pass # нужно сделать создание резюме
    else:
